...
export function myDataReducer(state: any = initialState, action: ActionType<any>) {
  switch (action.type) {
    ...
    case CHANGE_EXISTING_MESSAGE_SHARED_STATUS:
      return state
        .setIn(['messageForEdit', 'sharedStatus'], action.payload);

    case CHANGE_EXISTING_MESSAGE_TEXT:
      return state
        .setIn(['messageForEdit', 'text'], action.payload);
